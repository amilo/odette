...
//variables for sound triggering

boolean PLAYL, PLAYR = false;   
boolean FLYUPL,FLYUPR= true;
float t1L, t1R, t2L, t2R;
float nt1L, nt1R, nt2L, nt2R;
...
void setup()
{
  ...
  supercollider=new NetAddress("127.0.0.1", 57120);  //check the address with SuperCollider
...
if (frameCount%3==0){
  t1L=theta1L;   //store the angles t=0;
  t1R=theta1R;
  t2L=theta2L;   //store the angles t=0;
  t2R=theta2R;
}
if (frameCount%3==2){
  nt1L=theta1L;   //store the angle t=2;
  nt1R=theta1R;
  nt2L=theta2L;   //store the angle t=2;
  nt2R=theta2R;
  ...
  if (v1L>threshold) {     //treshold for speed, trigger sound
    FLYUPL = true;
    PLAYL = true;
      }
  else if (v1L<-threshold) {
    FLYUPL = false;
    PLAYL = true;
    v1L=-v1L; //absolute value for sending
     }
  ...
  else if (v1R<-threshold) {
    FLYUPR = false;
    PLAYR = true;
    v1R=-v1R; //absolute value for sending
         }
  
  if (PLAYL == true||PLAYR == true){
               
  OscMessage  msg = new OscMessage("/kinect");
  msg.add(map(theta1L,0,width,-6,6));
  msg.add(map(v1L,0,30,0,1));          //check the maximum optimal range for mapping th speed
  msg.add(map(theta1R,0,height,-6,6));
  msg.add(map(v1R,0,30,0,1));
  msg.add(map(theta2L,0,180,-2,2));   //map forearm for octave shifting
  msg.add(map(theta2R,0,180,-2,2));
  
  
  osc.send( msg,supercollider);
  //println("message sent");
  
  //leave the speed as it was to avoid errors
  if (FLYUPL = false){
  v1L=-v1L; 
  }
  if (FLYUPY = false){
